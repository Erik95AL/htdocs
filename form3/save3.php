...
<body>
    ...
    <div class="main">
        <h1 class="back"> Página de inicio<a id="home" class="anchor"></a></h1>
        <h2></h2>
        <h2></h2>
        ...
        <div>
            ...
            if (!$error) {
                if ($save1 & $save2 & $save3) {
                    $cryp = encrypt($password, $token, date('y.m.d'));
                    $cryp2 = encrypt($email, $token, date('y.m.d'));


                    $usuarios = new SimpleXMLElement('db3.xml', 0, true);
                    $nuevoUsuario = $usuarios->addChild('user');
                    $nuevoUsuario->addChild('name', $name);
                    $nuevoUsuario->addChild('email', $cryp2);
                    $nuevoUsuario->addChild('password', $cryp);
                    ...
                    // echo '//$save=false';
                    if (!$xml = simplexml_load_file('db3.xml')) {
                        ...
                        foreach ($xml as $user) {
                            ...
                            if ($user->email == encrypt($email, $token, $t)) {
                                if ($user->password == encrypt($password, $token, $t)) {
